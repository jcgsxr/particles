...
void CSplashScreen::init()
{
	...
	GET_IMGLOADER->loadImagePack(image_pack_splash, (int)(sizeof(image_pack_splash) / sizeof(uint32)));
	
	// If there exists more than one splash image, they must all be initialized here.
	_splash_images[0].init();
	_splash_images[0].loadSpriteImage(GET_IMGLOADER->getImage(FILE_ID_IMAGE_LLAMAFACE_SPLASH));
